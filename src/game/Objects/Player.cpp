...
//添加技能不添加天赋数组,用于天赋切换
//ientium@sina.com 小脏手修改
bool Player::addTalentSpell(uint32 spell_id, bool active, bool learning, bool dependent, bool disabled)
{
...
//********************************************************************************************************************************	
	...
	_LoadActions(holder->GetResult(PLAYER_LOGIN_QUERY_LOADACTIONS));//载入活动按钮方案
 //************************************************************************************************************************************
...
void Player::ActivateSpec(uint8 specNum)
{
	...
	for (PlayerTalentMap::const_iterator tempIter = tempSpec.begin(); tempIter != tempSpec.end(); ++tempIter)
	{
		...
		if (!GetTalentSpellPos(talentSpellId))
		{
			uint32 leanding = addTalentSpell(talentSpellId, true, false, false,false);
			if (leanding == true) {
				tempSpec.erase(tempIter++);
			}
			sLog.outError("Player::_LoadSpells: %s has talent spell %u in character_spell, removing it.",
				GetGuidStr().c_str(), talentSpellId);
			/*CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = '%u'", spell_id);*/
			continue;
		}
	}
